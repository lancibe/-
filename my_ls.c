...
#define  PARAM_L      2  //-l一行只显示一个文件的详细信息
#define  PARAM_R      4  //-R目录下的所有文件及子目录下的所有文件 
#define  MAXROWLEN   80  //一行最多显示的字符数

int g_leave_len = MAXROWLEN;//一行剩余长度，用于输出对对齐 
int g_maxlen; 			 //存放某目录下最长文件名的长度
int flag_param = PARAM_NONE;//参数种类，即是否有-l,-a和-R选项 
...
void colorprintf(char* name,mode_t st_mode);
static void mask_ctrl_c(void);  //屏蔽信号
...
int main(int argc, char** argv)
{
	...
	for(i = 1 ; i < argc ; i++)
	{
		if(argv[i][0] == '-')
		{
			...
			{
				param[j] = argv[i][k]; //获取-后面的参数 
			}
			num++;//保存-的个数 
		}
	} 
	
	/*只支持参数a、R和l，如果有其他选项就报错*/
	for(i = 0 ; i < j ; i++)
	{
		if(param[i] == 'a')
		{
...
void display_dir(int flag_param, char* path)
{
	...
	//使用冒泡法对文件名进行排序，排序后的文件名按字母顺序存储于filenames 
	for(i = 0 ; i < count - 1 ; i++)
	{
